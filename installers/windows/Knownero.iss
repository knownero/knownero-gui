...
[Setup]
AppName=Knownero GUI Wallet
; For InnoSetup this is the property that uniquely identifies the application as such
; Thus it's important to keep this stable over releases
; With a different "AppName" InnoSetup would treat a mere update as a completely new application and thus mess up

AppVersion=0.12.0.0
DefaultDirName={pf}\Knownero GUI Wallet
DefaultGroupName=Knownero GUI Wallet
UninstallDisplayIcon={app}\knownero-wallet-gui.exe
...
; As of version 0.12.0.0, the Knownero .exe files do not carry version info anyway in their .exe headers.
...
; applications somehow. Luckily this is no issue here because ALL files are "private" to Knownero.

Source: "ReadMe.htm"; DestDir: "{app}"; Flags: ignoreversion
Source: "FinishImage.bmp"; Flags: dontcopy

; Knownero GUI wallet
Source: "bin\knownero-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Knownero GUI wallet log file
...
Source: "knownero-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Knownero CLI wallet
Source: "bin\knownero-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Knownero wallet RPC interface implementation
Source: "bin\knownero-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion

; Knownero daemon
Source: "bin\knownerod.exe"; DestDir: "{app}"; Flags: ignoreversion

; Knownero daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "knownero-daemon.bat"; DestDir: "{app}"; Flags: ignoreversion;

; Knownero blockchain utilities
Source: "bin\knownero-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\knownero-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\knownero-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\knownero-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\knownero');
  s := 'The default folder to store the Knownero blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\knownerod.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "knownero-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Knownero daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\knownerod.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\knownero-daemon.bat'), s, false);
  end;
end;
...
[Icons]
; Icons in the "Knownero GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\knownero-wallet-gui.exe"
...
Name: "{group}\Utilities\Knownero Daemon"; Filename: "{app}\knownerod.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\knownero-wallet-cli.exe"; WorkingDir: "{userdocs}\Knownero\wallets"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Knownero\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\knownero-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\knownero-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM knownerod.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\knownero-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\knownero"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\knownero\knownero-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\knownero\knownero-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
